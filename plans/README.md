...
## Follow-up pass (all four lanes selected) — ✅ ALL SHIPPED (Phases D–H)

Status: D ✅ `69d3e83` · E ✅ `387d71f` · F ✅ `763111f` · G ✅ `5a90a02` · H ✅ `4e4b628`.
Test suite grew 51 → **167**; codebase is `any`-free; every phase verified with a live PNG+MP4 export smoke. Notable QA catches fixed in-flight: the NTSC fps muxer bug (Phase B) and a React #418 hydration mismatch from URL-state seeding (Phase H).

Gated phases below, each committed → auto-deployed:
